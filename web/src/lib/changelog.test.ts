...
describe('Changelog Milestones Data', () => {
  it('contains current version 1.4.0 as the first release milestone', () => {
    expect(CHANGELOG_RELEASES[0].version).toBe(APP_VERSION);
    expect(CHANGELOG_RELEASES[0].version).toBe('1.4.0');
  });

  it('contains all 5 historical milestones: v1.4.0, v1.3.0, v1.2.0, v1.1.0, v1.0.0', () => {
    const versions = CHANGELOG_RELEASES.map((r) => r.version);
    expect(versions).toEqual(['1.4.0', '1.3.0', '1.2.0', '1.1.0', '1.0.0']);
  });
