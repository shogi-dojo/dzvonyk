...
export function runPreflight(input: PreflightInput): PreflightResult {
  ...
  for (const rid of allDemandedRooms) {
    ...
    if (supply <= 0) {
      // A hard pin with no slots is unschedulable; a mere subject preference
      // is not, so it only warrants a warning.
      if (hard > 0) {
        blocking.push({
          code: 'ROOM_NO_SLOTS',
          severity: 'blocking',
        ...
        warnings.push({
          ...
          severity: 'warning',
          entity: { kind: 'room', id, name },
          message: tr('preflight.roomNoSlotsSoft', { name }),
        });
      }
      continue;
    }
    ...
    if (hard > supply) {
      blocking.push({
        ...
        severity: 'blocking',
        entity: { kind: 'room', id, name },
        message: tr('preflight.roomOverload', { name, demand: hard, supply }),
      });
      ...
      const message = demand > supply
        ? tr('preflight.roomOversubscribed', { name, demand, supply })
        : tr('preflight.roomNearCapacity', {
