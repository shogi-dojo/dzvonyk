...
describe('preserveInstitutionType', () => {
  it('keeps the college type when a .fet import replaces the rules row', () => {
    ...
    expect(preserveInstitutionType({ institutionType: 'college' }, null)).toBe('college');
  });
  ...
  it('lets the school win over a stale rules mirror', () => {
    expect(
      preserveInstitutionType({ institutionType: 'college' }, { institutionType: 'school' })
    ).toBe('college');
  });
});
